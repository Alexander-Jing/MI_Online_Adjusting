...
def Online_updating_EEGNet_simulation(args_dict):
    ...
    trial_pre_sessions = args_dict.trial_pre_sessions
    ...
    for trial_idx in range(trial_nums):
        ...
        if (trial_idx + 1) % update_wholeModel == 0:
            ...
            for label in unique_labels:
                indices = np.where(combined_label_array == label)[0]
                indices = indices[-(trial_pre_sessions + int(_update_wholeModel/2)*batch_size_online):]  # for faster training, only using part of the existing data 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--trial_pre_sessions', default=120, type=int, help="number of samples each class from previous sessions")
    ...
    trial_pre_sessions = args.trial_pre_sessions
    ...
    args_dict.trial_pre_sessions = trial_pre_sessions
