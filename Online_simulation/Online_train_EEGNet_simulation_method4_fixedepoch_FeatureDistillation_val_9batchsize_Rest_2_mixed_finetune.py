...
def Online_updating_EEGNet_simulation(args_dict):
    ...
    # the finetuning method is widely used in most online updating methods
    ...
    for trial_idx in range(trial_nums):
        ...
        if (trial_idx + 1) % update_wholeModel == 0:
            ...
            for label in unique_labels:
                indices = np.where(combined_label_array == label)[0]  # get the indices of the label, we use all the data to finetune the model
