...
from helpers.utils import seed_everything, makedir_if_not_exist, plot_confusion_matrix, \
    save_pickle, load_pickle, train_one_epoch, train_one_epoch_fea, train_update, eval_model, eval_model_fea, train_one_epoch_MMD, save_training_curves_FixedTrainValSplit, \
...
from helpers.utils import Offline_write_performance_info_FixedTrainValSplit, Offline_write_performance_info_FixedTrainValSplit_ConfusionMatrix, \
    ...
    accuracy_perclass_save2csv, accuracy_perclass_iteration_plot, eval_model_fea_exemplars, eval_model_fea_exemplars_distillation, train_one_epoch_fea_distillation, train_one_epoch_fealogitlabel_distillation, \
        eval_model_fea_exemplars_distillation_label, eval_model_fea_exemplars_distillation_datafea_logitlabel, train_one_epoch_logit_distillation, train_one_epoch_label_distillation, train_one_epoch_logitlabel_distillation, train_one_epoch_fea_MMDContrastive, train_one_epoch_fea_MMDContrastive_targetcls_iter_t, \
...
def Online_updating_EEGNet_simulation(args_dict):
    ...
    memoryBank_sources = {}  # for saving the memoryBank_source for further study
        ...
        if (trial_idx + 1) % update_trial == 0:           
            ...
            if (trial_idx+1) % update_wholeModel == 0:
                ...
                for epoch in trange(_n_epoch_online, desc='online classification update whole model'):
                    ...
                    average_loss_this_epoch = train_one_epoch_fea_MMDContrastive_targetcls_iter_t(model, optimizer, criterion, source_train_loader, target_train_loader, memoryBank_source, memoryBank_target, device, cons_beta=cons_rate)
                    ...
                    if whole_model_is_best:
                        ...
                        memoryBank_sources[trial_idx] = memoryBank_source
    ...
    save_pickle(result_save_subjectdir, "memoryBank_sources.pkl", memoryBank_sources)
...
def Online_data_painiting(args_dict):
    ...
    # load the memoryBank_sources dict
    experiment_name = restore_file
    result_save_subjectdir = os.path.join(Online_result_save_rootdir, sub_name, experiment_name)
    memoryBank_sources = load_pickle(result_save_subjectdir, "memoryBank_sources.pkl")  
    memoryBank_sources_tsne = {}
    
    # TSNE visualization
    for i, (trial, source) in enumerate(memoryBank_sources.items()):
        _X = source.reshape(source.shape[0], -1)
        _X_tsne = TSNE(n_components=2, learning_rate='auto', init='random', random_state=42, perplexity=1).fit_transform(_X)
        memoryBank_sources_tsne[i] = _X_tsne
    ...
    for i in range(8):
        ...
        col = i % 4
        ax = axes[row, col]
        _memoryBank_sources_tsne = memoryBank_sources_tsne[i]
        for j in range(_memoryBank_sources_tsne.shape[0]):
            ax.scatter(_memoryBank_sources_tsne[j, 0], _memoryBank_sources_tsne[j, 1], color=colors[j], label=f'Point {j+1}')
        
        ax.set_title(f'Trial {i+1}')
        ax.set_xlabel('Dimension 1')
        ax.set_ylabel('Dimension 2')
    plt.savefig(f'{result_save_subjectdir}/memoryBank_sources_tsne.png')
    plt.close()
    
    # caculate the mean 2-norm distance between the centers
    dis_protype = []
    for i, (trial, source) in enumerate(memoryBank_sources.items()):
        _X = source.reshape(source.shape[0], -1)
        _distances = []
        for _i in range(_X.shape[0]):
            for _j in range(_i+1, _X.shape[0]):
                _dist = np.linalg.norm(_X[_i, :] - _X[_j, :])
                _distances.append(_dist)
        dis_protype.append(np.mean(_distances))

    # plot the distance between the centers
    plt.figure(figsize=(16, 9))
    plt.plot(dis_protype, color='#838482', linewidth=4.0)
    plt.xlabel("iterations")
    plt.ylabel("distances")
    plt.savefig(f'{result_save_subjectdir}/dis_protype.png')
    plt.savefig(f'{result_save_subjectdir}/dis_protype.pdf')
    plt.close()
